...
text_path = "PyBank/Final_Analysis.txt"
...
with open(text_path, 'w') as file:
        ...
        file.write(f"Greates Increase in Proifits: {totalmonths[pc.index(max(pc))+1]}\n")
        # file.write("Greatest Increase in Profits: %s ($%s)\n" % ((totalmonths[pc.index(max(pc))+1])))
        file.write(f"Greates Decrease in Proifits: {totalmonths[pc.index(min(pc))+1]}")
